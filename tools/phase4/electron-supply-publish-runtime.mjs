...
async function run() {
  ...
  try {
    ...
    assert(publishRecords.some((record) => record.title === '发布夹具标题' && record.description === '发布夹具描述' && record.price === '88.5' && record.imageCount === '1' && record.sku.includes('颜色: 黑色') && !record.sku.includes('{')), `结构化 SKU 未完整回填: ${JSON.stringify(publishRecords)}`)
    ...
    console.log(JSON.stringify({ scenario: 'phase4-electron-supply-publish-runtime', migrations: migrationFiles, assertions: { actualElectron: true, persistentChromeProfile: true, batchClaimFilledAndSubmitted: true, structuredSkuFilledWithoutJson: true, randomAddressNoAdjacentRepeat: true, cloudSuccessReceipt: true, localOutboxDelivery: true, sensitiveBoundary: true, temporaryImageCleanup: true } }, null, 2))
  } finally {
