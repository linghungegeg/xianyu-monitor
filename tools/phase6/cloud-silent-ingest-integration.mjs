...
async function run() {
  ...
  try {
    ...
    const sensitive = await collectorApi.inject({ method: 'POST', url: '/v1/ingest', headers: auth(session.accessToken), payload: { ...payload, batchId: randomUUID(), idempotencyKey: 'phase6-sensitive', batchSequence: 3, records: [{ ...records[0], cookie: 'local-only' }] } })
    ...
    await db.query(`INSERT INTO identity.admin_users (id,email_normalized,password_hash,role,status,mfa_state,created_at) VALUES ($1,'admin-phase6@example.test',$2,'owner','active','enrolled',now())`, [randomUUID(), await hashPassword('phase6-admin-password-123')])
    const adminLogin = await adminApi.inject({ method: 'POST', url: '/v1/auth/login', payload: { email: 'admin-phase6@example.test', password: 'phase6-admin-password-123' } })
    ...
    const afterRevoke = await collectorApi.inject({ method: 'POST', url: '/v1/ingest', headers: auth(session.accessToken), payload: { ...payload, batchId: randomUUID(), idempotencyKey: 'phase6-revoked', batchSequence: 4 } })
    ...
    console.log(JSON.stringify({ scenario: 'phase6-cloud-silent-ingest', migrations, assertions: { migrationRepeatable: true, singleAndBatchSchema: true, batchAndRecordIdempotency: true, cursorAndResultTraceability: true, sensitivePayloadRejected: true, publicMediaMetadataOnly: true, userFinalMarketOnly: true, adminAuditFilters: true, revokedDeviceBlocked: true } }, null, 2))
  } finally {
