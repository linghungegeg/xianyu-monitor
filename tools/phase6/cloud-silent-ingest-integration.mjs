...
async function run() {
  ...
  try {
    ...
    const distinctKeysDuplicate = await collectorApi.inject({ method: 'POST', url: '/v1/ingest', headers: auth(session.accessToken), payload: { ...payload, batchId: randomUUID(), idempotencyKey: 'phase6-batch-snapshot-event-duplicate', batchSequence: 9, records: [{ ...records[2], idempotencyKey: 'snapshot:phase6:other-device' }, { ...records[3], idempotencyKey: 'event:phase6:other-device' }] } })
    assert(distinctKeysDuplicate.statusCode === 200 && json(distinctKeysDuplicate).insertedCount === 0 && json(distinctKeysDuplicate).deduplicatedCount === 2, '同快照或同事件的不同设备键未去重')
    const concurrentPayload = { ...payload, batchId: randomUUID(), idempotencyKey: 'phase6-concurrent-batch', batchSequence: 10 }
    const concurrent = await Promise.all([collectorApi.inject({ method: 'POST', url: '/v1/ingest', headers: auth(session.accessToken), payload: concurrentPayload }), collectorApi.inject({ method: 'POST', url: '/v1/ingest', headers: auth(session.accessToken), payload: concurrentPayload })])
    assert(concurrent.every((response) => response.statusCode === 200) && new Set(concurrent.map((response) => json(response).batchId)).size === 1 && concurrent.some((response) => json(response).duplicate === true), '同设备并发重复批次未返回规范幂等结果')
    const sensitive = await collectorApi.inject({ method: 'POST', url: '/v1/ingest', headers: auth(session.accessToken), payload: { ...payload, batchId: randomUUID(), idempotencyKey: 'phase6-sensitive', batchSequence: 3, records: [{ ...records[0], xianyuCookie: 'local-only', xianyuToken: 'local-token', chromeProfilePath: 'C:\\profile', xianyuAccountState: 'local-account-state', qrLoginState: 'local-qr-state', sessionCredential: 'local-session', password: 'local-password' }] } })
    assert(sensitive.statusCode === 400 && !['local-only', 'local-token', 'C:\\profile', 'local-account-state'].some((secret) => sensitive.body.includes(secret)), '敏感字段未拒绝或泄漏到响应')
