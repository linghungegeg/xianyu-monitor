...
async function run() {
  ...
  try {
    ...
    await first.page.locator('.launcher-auth-card').waitFor({ state: 'visible', timeout: 15_000 })
    ...
    assert(await first.page.locator('.topbar, .logs-panel').count() === 0, '未登录时仍显示工作台内容')
    await first.page.getByRole('button', { name: '登录' }).click()
    await first.page.getByText('请输入账号', { exact: true }).waitFor({ state: 'visible', timeout: 5_000 })
    assert(await first.page.locator('.launcher-remember').evaluate((element) => getComputedStyle(element).justifyContent) === 'flex-end', '记录账号密码登录未右对齐')
    await first.page.getByLabel('账号').fill(email); await first.page.getByLabel('密码').fill(password); await first.page.getByRole('button', { name: '登录' }).click()
