...
type RowStatus = '正常' | '关注' | '失败' | '重试中' | '已停用'
...
const samples: Record<AdminResource, Omit<Row, 'id' | 'updatedAt'>[]> = {
  users: [{ title: 'user_1024', detail: '专业版 · 2 台设备', metric: '最近活跃 8 分钟前', status: '正常', tag: '用户' }, { title: 'collector-win-01', detail: 'user_1024 · Windows', metric: '设备密钥有效', status: '正常', tag: '设备' }, { title: 'user_2048', detail: '基础版 · 需要关注', metric: '设备绑定请求', status: '关注', tag: '用户' }],
  billing: [{ title: 'ORD-20260817-001', detail: '专业版月付 · user_1024', metric: '¥99.00', status: '正常', tag: '订单' }, { title: 'LED-000203', detail: 'AI 分析扣费 · 幂等命中', metric: '1 次调用', status: '正常', tag: '账本' }, { title: '权益调整失败', detail: '人工授权', metric: '失败原因：授权未完成', status: '失败', tag: '权益' }],
  ...
  quality: [{ title: '数码类目字段完整度', detail: '手机 / 电脑 / 相机', metric: '98.6%', status: '正常', tag: '规则' }, { title: '价格异常样本', detail: '笔记本电脑 · 全国', metric: '12 条异常样本', status: '关注', tag: '异常' }, { title: '图片引用可用性', detail: '过去 24 小时', metric: '99.2%', status: '正常', tag: '媒体' }],
  uploads: [{ title: 'UPL-20260817-00821', detail: 'collector-win-01 · 250 条', metric: '去重 239，新增 11', status: '正常', tag: '批次' }, { title: '价格下调事件', detail: '商品版本比较', metric: '已写入事件链', status: '正常', tag: '事件' }, { title: 'UPL-20260817-00820', detail: '断网恢复补传', metric: '已重试 2 次，等待网络恢复', status: '重试中', tag: '批次' }],
...
function normalizeAdminRows(resource: AdminResource, page: CursorPage<Record<string, unknown>>): CursorPage<Row> {
    ...
    return {
      ...
      items: page.items.map((item) => {
        ...
        return {
          ...
          updatedAt: String(item.createdAt ?? ''), status: enabled ? '正常' : '已停用', tag: '用户', enabled, points: Number(item.points ?? 0), userData
        }
      })
    }
  }
  ...
  return {
    ...page,
    items: page.items.map((item) => {
      const status = String(item.qualityStatus ?? item.status ?? 'pending').toLowerCase()
      const rowStatus: RowStatus = status === 'passed' || status === 'completed' || status === 'succeeded' || status === 'active'
        ? '正常'
        : status === 'failed' || status === 'rejected' || status === 'error'
          ? '失败'
          : status === 'retrying' || status === 'retry' || status === 'requeued'
            ? '重试中'
            : status === 'disabled' || status === 'revoked' || status === 'inactive'
              ? '已停用'
              : '关注'
...
function List(props: { resource: AdminResource; view: { title: string; description: string; columns: [string, string, string, string] }; page: CursorPage<Row> | null; cursorIndex: number; query: string; status: string; sort: SortKey; limit: 20 | 50 | 100; loading: boolean; failure: string | null; notice: string | null; onQuery: (value: string) => void; onStatus: (value: string) => void; onSort: (value: SortKey) => void; onLimit: (value: 20 | 50 | 100) => void; onPrev: () => void; onNext: () => void; onRefresh: () => void; onOpen: (row: Row) => void }): ReactNode {
  ...
  return <>
    ...
    <section className="filters"><label><Search size={16} /><input value={query} onChange={(event) => props.onQuery(event.target.value)} placeholder="搜索当前范围" /></label><select aria-label="状态" value={status} onChange={(event) => props.onStatus(event.target.value)}><option value="">全部状态</option><option>正常</option><option>关注</option><option>失败</option><option>重试中</option><option>已停用</option></select><select aria-label="排序" value={sort} onChange={(event) => props.onSort(event.target.value as SortKey)}><option value="updated_at">最近更新</option><option value="status">状态优先</option><option value="title">名称</option></select><span /><button className="icon" title="刷新当前页" onClick={props.onRefresh}><RefreshCw className={loading ? 'spin' : ''} size={17} /></button></section>
